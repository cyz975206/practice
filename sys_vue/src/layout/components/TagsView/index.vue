<template>
  <div class="tags-container">
    <scroll-pane ref="scrollPaneRef" @scroll="handleScroll">
      <router-link
        ref="tagRef"
        v-for="tag in filteredVisitedViews"
        :key="tag.fullPath"
...
let authMenus=userStore?.userInfo?.authMenus||[] //当前用户的菜单权限
//显示tag
const filteredVisitedViews = computed(() => {
  return visitedViews.value.filter(view => {
    // 有些view可能没有menuId，先判断是否存在
    const menuId = view.meta?.menuId;
    if (!menuId) return true; // 没有menuId的视图默认显示
    return authMenus.includes(menuId);
  });
});
...
watch(
  ...
  () => {
    // 应用详情不加入tagView中
    // if (
    //   route.path.includes("/app/detail") ||
    //   route.path.includes("/app/answer") ||
    //   route.path.includes("/app/session")
    // ) {
    //   return;
    // }
    addTags();
...
function filterAffixTags(routes: RouteRecordRaw[], basePath = "/") {
  const processRoute = (route: RouteRecordRaw) => {
    const fullPath = (basePath.replace(/\/$/, "") + "/" + route.path).replace(/\/+/g, "/");
